    ...
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm md:text-base">
            <thead class="bg-gray-50 text-black sticky top-0 z-10
                ...
                <tr>
                    <th>ลำดับ</th>
                    <th>ชื่อเวอร์ชันเกณฑ์การประเมิน</th>
...
const toThaiDate = (iso) => {
    if (!iso) return '-';
    try { return new Intl.DateTimeFormat('th-TH', { dateStyle: 'short' }).format(new Date(iso)); }
    catch { return '-'; }
};
...
function fetchCriteriaVersions() {
    ...
    .then(r => r.json())
    .then(({data}) => renderCriteriaTable(data))
    .catch(() => {
...
function renderCriteriaTable(items) {
    const tbody = document.getElementById('criteria-table-body');
    tbody.innerHTML = '';
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="7" class="text-center py-10 text-gray-500">
                    <i class="fas fa-list text-3xl mb-2 block"></i>
                    <p>ไม่พบข้อมูลเกณฑ์การประเมิน</p>
                </td>
            </tr>`;
        return;
    }
    ...
    items.forEach((item, idx) => {
        const title = (item.version_name || '').trim();
        ...
        tr.addEventListener('click', function(e) {
            // ป้องกันคลิกที่ปุ่ม/ลิงก์ในคอลัมน์แอ็กชัน
            if (e.target.closest('.action-stop')) return;
            window.location.href = showUrl;
        });

        const assessmentType = (item.assessment_type || '-').toString();
        tr.dataset.type = assessmentType.toLowerCase();

        // Style of badge 
        const typeBadgeClass = assessmentType === 'วิชาการ' ? 'px-2 py-1 rounded-full text-sm font-normal ring-1 bg-yellow-100 text-yellow-700 ring-yellow-200' 
        : assessmentType === 'สนับสนุน' ? 'px-2 py-1 rounded-full text-sm font-normal ring-1 bg-orange-100 text-orange-700 ring-orange-200' : 'bg-gray-100 text-gray-700 ring-1 ring-inset ring-gray-200';

        tr.innerHTML = `
            <td class="px-6 py-3 font-normal text-black text-center">${idx + 1}</td>
            <td class="px-6 py-3 text-black">
                <a href="${showUrl}" class="decoration-transparent group-hover:underline">
                    ${title || 'ไม่มีการระบุชื่อเวอร์ชัน'}
                </a>
            </td>
            <td class="px-6 py-3">
                <span class="inline-flex items-center px-2.5 py-1 rounded-md text-sm font-medium ${typeBadgeClass}">
                    ${assessmentType}
                </span>
            </td>
            <td class="px-6 py-3 text-black">${updatedAt}</td>
            <td class="px-6 py-3 text-black">${creatorName}</td>
            <td class="px-6 py-3 text-black">${createdAt}</td>
            <td class="px-6 py-2">
        ...
        // bind ลบ
        tr.querySelector('button.action-stop')?.addEventListener('click', function(e) {
            e.stopPropagation();
            showDeleteModal(this.dataset.id, this);
        });
...
function ensureDeleteModal() {
    ...
    deleteModal.className = 'fixed inset-0 z-[99999] flex items-center justify-center hidden';
...
// ---------- Init + client-side search (ถ้ามี #search-input) ---------- //
            ...
            t = setTimeout(() => {
                ...
                if (!anyVisible) {
                    if (!noRow) {
                        const tr = document.createElement('tr');
                        tr.id = 'no-result-row';
                        tr.innerHTML = `<td colspan="6" class="px-6 py-6 text-center text-base text-gray-600">ไม่พบข้อมูลเกณฑ์การประเมินที่ตรงกับคำค้น</td>`;
                        tbody.appendChild(tr);
                    }
                } else {
                    noRow?.remove();
